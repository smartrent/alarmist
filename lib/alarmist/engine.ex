...
defmodule Alarmist.Engine do
  ...
  * `:lookup_fun` - function for looking up alarm state
  ...
  @doc """
  Create and add a synthetic alarm based on a condition specification
  ...
  """
  @spec add_synthetic_alarm(t(), Alarmist.alarm_id(), Alarmist.compiled_condition()) :: t()
  def add_synthetic_alarm(engine, alarm_id, compiled_condition) do
    engine
  ...
  defp register_condition(engine, alarm_id, compiled_condition) do
    %{engine | registered_conditions: Map.put(engine.registered_conditions, alarm_id, compiled_condition)}
  end

  defp link_rules(engine, rules, synthetic_alarm_id) do
    new_engine =
      Enum.reduce(rules, engine, fn rule, engine ->
        link_rule(engine, rule, synthetic_alarm_id)
      end)
  ...
  def remove_synthetic_alarm(engine, synthetic_alarm_id) do
    {condition, new_registered_conditions} =
      Map.pop(engine.registered_conditions, synthetic_alarm_id)

    alarm_ids_to_clear = [synthetic_alarm_id | condition.temporaries]
