defmodule Alarmist.Rules.Flapping do
  ...
  def on_set({:flapping, name, options}, _monitor_state) do
    ...
    if current_counter_value >= threshold do
      # Counter is at the configured threshold, raise the alarm
      [{:raise, name}, {:increment_counter, name}]
    else
  ...
  @impl Rule
  @spec on_clear({:flapping, any(), any()}, any()) :: [{:clear, any()}, ...]
  def on_clear({:flapping, name, _options}, _monitor_state) do
    [{:clear, name}]
  end

  @impl Rule
  def on_check({:flapping, name, _options}, _monitor_state) do
    [{:reset_counter, name}]
  end
end
